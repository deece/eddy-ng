...
def test_cmd_setup_auto_contact(eddy_env):
    eddy = eddy_env.eddy
    eddy._temp_sensor_obj = None
    
    class KlipperCommandError(Exception):
        pass
    
    # Mock printer command_error class to avoid TypeError
    eddy_env.printer.command_error = KlipperCommandError
    
    gcmd = MagicMock()
    gcmd.get.side_effect = lambda name, default=None: {
        "FILAMENT": "pla",
    }.get(name, default)
    gcmd.get_int.side_effect = lambda name, default=None: {
        "NOHOME": 1,
    }.get(name, default)
    gcmd.get_float.side_effect = lambda name, default=None: {
        "TARGET_TEMP": 0.0,
    }.get(name, default)

    # Mock toolhead and kinematics
    th = MagicMock()
    th_pos = [60.0, 60.0, 115.0]
    th.get_position.side_effect = lambda: list(th_pos)
    
    # Mock rails range lookup
    rail_z = MagicMock()
    rail_z.get_range.return_value = (0.0, 120.0)
    kin = MagicMock()
    kin.rails = [MagicMock(), MagicMock(), rail_z]
    th.get_kinematics.return_value = kin
    
    def mock_manual_move(pos, speed):
        if pos[2] is not None:
            th_pos[2] = pos[2]
    th.manual_move.side_effect = mock_manual_move

    # Mock _z_homed and _xy_homed
    eddy._z_homed = MagicMock(return_value=False)
    eddy._xy_homed = MagicMock(return_value=True)
    eddy.cmd_SETUP_next = MagicMock()

    # Mock read_one_value to simulate proximity frequency increase and bed contact
    def mock_read_one_value():
        val = MagicMock()
        z = th_pos[2]
        if z > 25.0:
            ...
            # Rise sharply closer to the bed, matching physical characteristics
            val.freq = 1318000.0 + 20000.0 / (z - 18.0) - (20000.0 / 7.0)
        else:
            # Plateaus upon contact (nozzle resting on bed)
            val.freq = 1318000.0 + 20000.0 / (20.0 - 18.0) - (20000.0 / 7.0)
        val.status = 0
        return val
    eddy._sensor.read_one_value.side_effect = mock_read_one_value

    # Track gcode calls
    eddy_env.eddy._gcode.run_script_from_command = MagicMock()

    # Patch lookup_object on printer
    eddy_env.printer.lookup_object.side_effect = lambda name, default=None: {
        "toolhead": th,
        "configfile": eddy_env.configfile,
    }.get(name, MagicMock())

    # Mock _set_toolhead_position to update th_pos
    ...
    # Verify that contact was detected, Z coordinate was zeroed, and cmd_SETUP_next was called
    assert eddy.cmd_SETUP_next.call_count == 1
    eddy.cmd_SETUP_next.assert_called_once_with(gcmd, [0.0, 0.0, 0.0])
    assert th_pos[2] == 0.0
    
    # Verify no SET_TMC_CURRENT was executed during cmd_SETUP
    for call in eddy_env.eddy._gcode.run_script_from_command.call_args_list:
        cmd = call[0][0]
        assert "SET_TMC_CURRENT" not in cmd


def test_cmd_setup_manual_z(eddy_env):
    eddy = eddy_env.eddy
    eddy._temp_sensor_obj = None
    ...
    # Patch manual_probe.ManualProbeHelper
    with patch.object(probe_eddy_ng.manual_probe, 'ManualProbeHelper') as mock_helper:
        eddy.cmd_SETUP(gcmd)
        mock_helper.assert_called_once()
        
        # Verify ManualProbeHelper callback behavior
        args, kwargs = mock_helper.call_args
        callback = args[2]
